...
pair at each higher ancestor. The first nonempty stage shadows every farther stage. Resolution stops
at the highest real folder, so unrelated top-level namespaces remain isolated.
...
Missing references remain ordinary source text.
...
- `GET /api/references/resolve?folder_id=...&tag=@group` returns `resolved`, `missing`, or
  ...
  formulation Markdown for hover preview.
- `GET /api/references/candidates?folder_id=...&q=...&limit=40` returns only lexically visible
  targets for the insertion picker. The UI inserts the returned `insert_text` verbatim.
...
normalized occurrence, contextual ancestor distance, authored order, then canonical tag.
...
Exact canonical and direct-scope lookups use hash maps; subtree stages use binary searches over
preorder-ranked target rows. Short-name resolution is therefore proportional to folder depth and
the matches in the first nonempty stage, not library size. Full-text query cost is proportional to
the smallest intersected posting lists plus verified matches. Very common terms can still approach
a full scan; the result cap bounds response size, not that honest worst case.
...
signature sweep of the already indexed Markdown paths at most once every 250 milliseconds. The
signature includes device, inode, size, modification time, and change time. Thus a manual Markdown
edit may remain visible through the old cache for at most 250 milliseconds and is refreshed on the
next query after that bound.
