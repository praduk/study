  ...
  return (
    <Dialog open={open} onOpenChange={(value) => !value && onClose()}>
      <DialogContent className="export-dialog" showCloseButton>
        ...
        <label className="field-label">Folder<select value={folderId} onChange={(event) => setFolderId(event.target.value)}><option value="">Entire library</option>{open && folders.map((folder) => <option key={folder.id} value={folder.id}>{folder.namespace} — {folder.name}</option>)}</select></label>
