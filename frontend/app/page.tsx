...
import { lazy, memo, Suspense, useCallback, useEffect, useMemo, useRef, useState, useSyncExternalStore, type CSSProperties } from 'react';
import {
...
function TreeNode({ node, selectedEntry, selectedFolder, expanded, pendingReviewPreferences, readOnly, onToggle, onSelectEntry, onSelectFolder, onReviewToggle, onEntryReviewToggle, onMove, onInsertEntry, onInsertFolder }: {
  ...
  const count = useMemo(() => folderEntryCount(node), [node]);
...
const LibraryTree = memo(function LibraryTree(props: Omit<React.ComponentProps<typeof TreeNode>, 'node'> & { tree: FolderNode[] }) {
...
export default function Home() {
  ...
  const updateReviewPreference = useCallback(async (type: 'folder' | 'entry', id: string, enabled: boolean) => {
  ...
  const beginCreateEntry = useCallback((folderId: string, index: number | null, kind: EntryKind = 'df') => { if (isMobile || !librarySynchronized || pendingReviewPreferencesRef.current.size) return; setSelectedFolderId(folderId); setCreateKind(kind); setCreateIndex(index); setCreateEntry(true); setEditorOpen(true); }, [isMobile, librarySynchronized]);
  ...
  const toggleFolder = useCallback((id: string) => setExpanded((current) => {
    const next = new Set(current);
    if (next.has(id)) next.delete(id);
    else next.add(id);
    return next;
  }), []);
  const toggleFolderReview = useCallback((id: string, enabled: boolean) => {
    void updateReviewPreference('folder', id, enabled);
  }, [updateReviewPreference]);
  const toggleEntryReview = useCallback((id: string, enabled: boolean) => {
    void updateReviewPreference('entry', id, enabled);
  }, [updateReviewPreference]);
  ...
  const treeProps = { tree: data?.tree || [], selectedEntry: selectedEntryId, selectedFolder: selectedFolderId, expanded, pendingReviewPreferences, readOnly: isMobile || !librarySynchronized, onToggle: toggleFolder, onSelectEntry: chooseEntry, onSelectFolder: setSelectedFolderId, onReviewToggle: toggleFolderReview,
    onEntryReviewToggle: toggleEntryReview, onMove: moveItem, onInsertEntry: beginCreateEntry, onInsertFolder: addFolder };
