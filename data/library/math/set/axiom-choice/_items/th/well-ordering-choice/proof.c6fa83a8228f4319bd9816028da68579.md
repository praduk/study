Assume Choice and let $A$ be nonempty. Fix a choice function $c$ on the nonempty subsets of $A$. By @[transfinite recursion]math:set:ordinals:th:transfinite-recursion through the Hartogs ordinal $h(A)$, choose
...
Conversely, assume every set can be well-ordered. Given a set $\mathcal A$ of nonempty sets, @[well-order]math:set:orders-well-orders:df:well-order $U=\bigcup\mathcal A$. For each $A\in\mathcal A$, let $c(A)$ be the least element of $A$ in this well-order. Replacement produces the graph of $c$, and $c$ is a choice function. Thus Choice holds.
